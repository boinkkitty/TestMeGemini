...
> Stack: Next.js · Tailwind v4 · shadcn/ui · Geist Sans
...
| `--accent` | `oklch(0.96 0.025 245)` | Light blue tint — hover fills, active nav bg |
...
**Font:** Geist Sans (Next.js default via `next/font/local`)  
...
**Sidebar width:** 232px (fixed, `sticky top-0 h-screen`)  
...
- **User chip:** Avatar (28px circle, `bg-accent`), name + email, bottom of sidebar
...
- List: score ring (SVG circle progress), title, category badge, date, score/max, badge
- Detail: per-question review with correct/incorrect/missed highlighted, MRQ labelled

### Upload

- Single card form: Chapter Title + Category (2-col grid), PDF dropzone, Generate button
...
- ❌ Inter or Roboto — stick to Geist Sans
